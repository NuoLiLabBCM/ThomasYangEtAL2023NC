...
%% Fig. 6b norm. FR, early delay SC photoinhibition, ALM record
...
% 0.5 s
i_stim=1;
i_n_trial =  min(N_trials_all(:,[1 2 3 4]),[],2)>=3;
...
for i_pref = 1:2        % contra-selective cells vs. ipsi-selective
    ...
    if i_stim==1        % 0.5 s
        PSTH_stim_yes_tmp = PSTH_stim1_yes_aligned(i_cell,:);
        PSTH_stim_no_tmp = PSTH_stim1_no_aligned(i_cell,:);
    else                % 1.3s
    ...
    figure(1)
    subplot(2,4,1+(i_pref-1)*4+(i_stim-1)*2)
    ...
    subplot(2,4,2+(i_pref-1)*4+(i_stim-1)*2)
    ...
    ylim([0 3])
    if i_stim==1
        line([0 0.5]-1.3,[2.5 2.5],'color',[0 0.6 1],'linewidth',3)
    else
...
[y x] = hist(deltaFR_delay(i_contra_cells & conditions_all(:,1)==1,1),-2:.2:2);
bar(x,y,'b');
ylim_tmp = get(gca,'Ylim'); ylim(ylim_tmp);
line([0 0], ylim_tmp,'color','k');
plot(mean(deltaFR_delay(i_contra_cells & conditions_all(:,1)==1,1)),max(ylim_tmp)*.95,'vr')
subplot(2,2,3); hold on; title('lick ipsi trials')
[y x] = hist(deltaFR_delay(i_contra_cells & conditions_all(:,1)==0,1),-2:.2:2);
bar(x,y,'r');
ylim_tmp = get(gca,'Ylim'); ylim(ylim_tmp);
line([0 0], ylim_tmp,'color','k');
plot(mean(deltaFR_delay(i_contra_cells & conditions_all(:,1)==0,1)),max(ylim_tmp)*.95,'vr')
xlabel('delta FR')
...
[y x] = hist(deltaFR_delay(i_ipsi_cells & conditions_all(:,1)==1,1),-2:.2:2);
bar(x,y,'b');
ylim_tmp = get(gca,'Ylim'); ylim(ylim_tmp);
line([0 0], ylim_tmp,'color','k');
plot(mean(deltaFR_delay(i_ipsi_cells & conditions_all(:,1)==1,1)),max(ylim_tmp)*.95,'vr')
subplot(2,2,4); hold on; title('lick ipsi trials')
[y x] = hist(deltaFR_delay(i_ipsi_cells & conditions_all(:,1)==0,1),-2:.2:2);
bar(x,y,'r');
ylim_tmp = get(gca,'Ylim'); ylim(ylim_tmp);
line([0 0], ylim_tmp,'color','k');
xlabel('delta FR')
...
subplot(2,4,1); title('ipsi SC stim, both ALM, lick-contra-preferring neurons')
subplot(2,4,5); title('ipsi SC stim, both ALM, lick-ipsi-preferring neurons')



disp('******************** SC silencing 1.3s ************************')
disp('============ contra population ===============')
i_contra_cells = (conditions_all(:,4)==1);
% [ trial_type(1-yes, 0-no)   stim(1 or 0)   session#   contra_ipsi(1-contra, 0-ipsi) ]
tbl = table(deltaFR_delay(i_contra_cells,1),conditions_all(i_contra_cells,1),conditions_all(i_contra_cells,3),'VariableNames',{'Activity','TrialType','Session'});
