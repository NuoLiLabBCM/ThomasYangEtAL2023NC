...
%% Fig. 5b norm. FR, SC photoactivation, ipsi ALM record



clear all
close all
% computing mixed effect model, using photoinhibition as main effect

addpath('../func/')


load Figure_5_b_data
...
for i_pref = 1:2        % contra-selective cells vs. ipsi-selective
    ...
    figure(10)
    subplot(2,2,1+(i_pref-1)*2+(i_stim-1)); hold on
    ...
    if i_stim==1
        line([0 0.5]-1.3,[1.2 1.2],'color',[0 0.6 1],'linewidth',3)
    else
    ...
    if i_stim==1
        line([0 0.5]-1.3,[.8 .8],'color',[0 0.6 1],'linewidth',3)
    else
