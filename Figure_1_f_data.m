...
for i_mice = 1:n_animals
    ...
    for i_SC_side = 1:2     % 1-leftSC, 2-rightSC
        ...
        for i_aom = [0 .1 .2 .3 .4 .5]
            ...
            if i_aom ==0
                ...
                R_early_lick(end+1,1) = sum(trial_selection & AOM_data_allSession == 0 & StimTrials_allSession >0 & LickEarly_allSession == 1 & Lick_Side_allSession==3)/sum(trial_selection & AOM_data_allSession == 0 & StimTrials_allSession == 1);
                L_early_lick(end+1,1) = sum(trial_selection & AOM_data_allSession == 0 & StimTrials_allSession >0 & LickEarly_allSession == 1 & Lick_Side_allSession==1)/sum(trial_selection & AOM_data_allSession == 0 & StimTrials_allSession == 1);
                ...
                if sum(trial_selection & stim_side_selection)>0
                    X_type(end+1,1) = size(X_type,1)+1;
                    R_early_lick(end+1,1) = sum(trial_selection & stim_side_selection & LickEarly_allSession == 1 & Lick_Side_allSession==3)/sum(trial_selection & stim_side_selection);
                    L_early_lick(end+1,1) = sum(trial_selection & stim_side_selection & LickEarly_allSession == 1 & Lick_Side_allSession==1)/sum(trial_selection & stim_side_selection);
        ...
        subplot(1,2,i_SC_side); hold on
        if length(n_trials)>1
            plot(X_type,R_early_lick,'-ob','markerfacecolor','w')
            plot(X_type,L_early_lick,'-or','markerfacecolor','w')
        end
...
subplot(1,2,1); title('left SC (sample or delay stim)')
xlabel('Power')
...
%% break out sample and delay stim conditions
figure
for i_mice = 1:n_animals
    
    
    for i_stim_epoch = 1:4
        ...
        for i_SC_side = 1:2     % 1-leftSC, 2-rightSC
            ...
            for i_aom = [0 .1 .2 .3]
                ...
                % select stim condition (Early late sample delay)
                if i_stim_epoch==1 % early sample
                    stim_epoch_selection = (Stim_Period_allSession==1 & StimDur_allSession ==0.5 & StimOnTime_allSession<1);
                elseif i_stim_epoch==2 % late sample
                    stim_epoch_selection = (Stim_Period_allSession==1 & StimDur_allSession ==0.5 & StimOnTime_allSession>1);
                elseif i_stim_epoch==3 % early delay
                    stim_epoch_selection = (Stim_Period_allSession==2 & StimDur_allSession ==0.5 & StimOnTime_allSession<2);
                elseif i_stim_epoch==4 % late delay
                    stim_epoch_selection = (Stim_Period_allSession==2 & StimDur_allSession ==0.5 & StimOnTime_allSession>2);
                end
                ...
                if i_aom ==0
                    ...
                    trial_selection = (AOM_data_allSession == i_aom & stim_epoch_selection & StimTrials_allSession >0 & Session_Index_allSession(:,1)==i_mice);
                    if sum(trial_selection & stim_side_selection)>0
                        X_type(end+1,1) = size(X_type,1)+1;
                        R_early_lick(end+1,1) = sum(trial_selection & stim_side_selection & LickEarly_allSession == 1 & Lick_Side_allSession==3)/sum(trial_selection & stim_side_selection);
                        L_early_lick(end+1,1) = sum(trial_selection & stim_side_selection & LickEarly_allSession == 1 & Lick_Side_allSession==1)/sum(trial_selection & stim_side_selection);
            ...
            subplot(4,2,(i_stim_epoch-1)*2+i_SC_side); hold on
            if length(n_trials)>1
                plot(X_type,R_early_lick,'-ob','markerfacecolor','w')
                plot(X_type,L_early_lick,'-or','markerfacecolor','w')
            end
...
subplot(4,2,1); title('left SC (early sample)')
xlabel('Power')
ylabel('Fraction of early licks')
ylim([0 1])
xlim([0 4])
...
subplot(4,2,3); title('late sample')
subplot(4,2,5); title('early delay')
subplot(4,2,7); title('late delay')
...
%% licking temporal histogram, individual mice, early sample stim only
...
for i_mice = 1:n_animals
    ...
    for i_aom = [0 0.1 0.3 0.4 0.5]
        ...
        for i_SC_side = 1:2
            ...
            % lick early
            if i_aom ==0
                trial_selection = (AOM_data_allSession == i_aom & StimTrials_allSession >0 & Session_Index_allSession(:,1)==i_mice & (R_hit_allSession==1 | L_hit_allSession==1));
            else
                trial_selection = (AOM_data_allSession == i_aom & stim_epoch_selection & StimTrials_allSession >0 & stim_side_selection & Session_Index_allSession(:,1)==i_mice);
            end
    ...
    subplot(5,2,1); title(['mouse',num2str(i_mice)])
