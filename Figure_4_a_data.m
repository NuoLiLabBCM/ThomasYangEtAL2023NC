...
% i_selective = ((sum(sig_selective(:,2),2)>0) & i_location_select); %i_selective = ((sum(sig_selective(:,1:2),2)>1) & i_location_select);      %<-------------- neuron selection
...
% 
% % 1.3 s
i_n_trial = N_trials_all(:,5)>=5 & N_trials_all(:,6)>=5 & N_trials_all(:,9)>=2 & N_trials_all(:,10)>=2; %1.3 s stim
